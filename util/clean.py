...
def clean_unwanted_characters(final_csv_path):
    ...
    def to_pinyin(text):
        text=lazy_pinyin(text, style=Style.NORMAL, errors='default', strict=True, v_to_u=False, neutral_tone_with_five=False,tone_sandhi=False)
        text = ' '.join(text)
        return text

    df_ds_final['transcript'] = df_ds_final['transcript'].apply(to_pinyin)

    #Save cleaned files
    final_path = final_csv_path[:-4]
    print('Length of ds_final: {}'.format(len(df_ds_final)))
    df_ds_final.to_csv('./merged_csv/'+final_path + '_pinyin.csv', header=True, index=False, encoding='utf-8')
