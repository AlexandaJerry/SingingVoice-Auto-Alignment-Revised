...
new_path='./my_corpus/'#新文件夹路径
os.mkdir(new_path)
wav_path='./my_wavs/'
lrc_path='./my_lyrics/'

for derName, subfolders, filenames in os.walk('./my_wavs/'):#要复制的文件夹所在的路径
  for i in range(len(filenames)):
    if filenames[i].endswith('.wav'):
        wav_file_path = wav_path + filenames[i].split('.')[0] + ".wav"
        lrc_file_path = lrc_path + filenames[i].split('.')[0] + ".txt"
        new_file_path = new_path + filenames[i].split('.')[0] + ".wav"
        news_file_path = new_path + filenames[i].split('.')[0] + ".txt"
        shutil.copy(wav_file_path, new_file_path)
        shutil.copy(lrc_file_path, news_file_path)
